...
BYTE CGuildManager::AddGuildRelationship(int index,DWORD dwSourceGuild,DWORD dwTargetGuild,BYTE btRelationType)
{
	...
	switch(btRelationType)
	{
		case 1:
			...
			gGuild.DGGuildUnionInfoSend(lpTargetGuild->dwNumber,1);

			UnionGuildAmount = this->GetUnionList(lpTargetGuild->dwNumber,UnionGuildNumber);

			gGuild.DGGuildUnionUpdateSend(0,UnionGuildAmount,(int*)UnionGuildNumber);
		...
		case 2:
			...
			gGuild.DGGuildUnionInfoSend(lpSourceGuild->dwNumber,2);

			gGuild.DGGuildUnionInfoSend(lpTargetGuild->dwNumber,2);

			RivalGuildNumber[RivalGuildAmount++] = lpSourceGuild->dwNumber;

			RivalGuildNumber[RivalGuildAmount++] = lpTargetGuild->dwNumber;

			gGuild.DGGuildUnionUpdateSend(0,RivalGuildAmount,(int*)RivalGuildNumber);
	...
	switch(btRelationType)
	{
		case 1:
			...
			if(UnionGuildAmount == 2)
			{
				...
				else
				{
					...
					gGuild.DGGuildUnionInfoSend(lpUnionInfo->dwNumber,1);
					gGuild.DGGuildUnionUpdateSend(16,UnionGuildAmount,(int*)UnionGuildNumber);
					return 1;
				}
			}
			else
		...
		case 2:
			...
			else
			{
				...
				gGuild.DGGuildUnionInfoSend(lpGuildInfo->dwNumber,2);
				gGuild.DGGuildUnionInfoSend(lpRivalInfo->dwNumber,2);
