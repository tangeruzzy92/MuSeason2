...
bool CMercenary::CreateMercenary(int aIndex,int MonsterClass,BYTE x,BYTE y) // OK
{
	...
	{
		gNotice.GCNoticeSend(iIndex,1,0,0,0,0,0,gMessage.GetMessage(128));
		return FALSE;
	}
	...
	if( iMercenaryTypeIndex == 286 || iMercenaryTypeIndex == 287)
	{
		...
		{
			gNotice.GCNoticeSend(iIndex,1,0,0,0,0,0,gMessage.GetMessage(129));
			return FALSE;
		}
		if(lpObj->GuildStatus != 0x80 && lpObj->GuildStatus != 0x40)
		{
			gNotice.GCNoticeSend(iIndex,1,0,0,0,0,0,gMessage.GetMessage(130));
				return FALSE;
		}
	}
	...
	{
		gNotice.GCNoticeSend(iIndex,1,0,0,0,0,0,gMessage.GetMessage(131));
		return FALSE;
	}
	...
	if(iMonsterIndex >= 0)
	{
		...
		gObj[iMonsterIndex].Attribute = ATTRIBUTE_NONE;
		gObj[iMonsterIndex].DieRegen = 0;
		gObj[iMonsterIndex].CsNpcType = 2;
		gObj[iMonsterIndex].CsJoinSide = 1;

		gNotice.GCNoticeSend(iIndex,1,0,0,0,0,0,gMessage.GetMessage(132));

		this->m_MercenaryCount++;

		if(lpObj->Guild != NULL)
		{
			LogAdd(LOG_GREEN,"[CastleSiege] Mercenary is summoned [%d] - [%d][%d] [%s][%s][%d] - (Guild : %s)",iMonsterIndex,iMercenaryTypeIndex,this->m_MercenaryCount,lpObj->Account,lpObj->Name,lpObj->GuildStatus,lpObj->Guild->Name);
		}
		else
		{
			LogAdd(LOG_GREEN,"[CastleSiege] Mercenary is summoned [%d] - [%d][%d] [%s][%s][%d]",iMonsterIndex,iMercenaryTypeIndex,this->m_MercenaryCount,lpObj->Account,lpObj->Name,lpObj->GuildStatus);
		}
	}
	else
	{
		gNotice.GCNoticeSend(iIndex,1,0,0,0,0,0,gMessage.GetMessage(133));
		return FALSE;
	}
...
bool CMercenary::DeleteMercenary(int aIndex) // OK
{
	...
	LogAdd(LOG_GREEN,"[CastleSiege] Mercenary is broken [%d] - [%d]",iIndex,this->m_MercenaryCount);
