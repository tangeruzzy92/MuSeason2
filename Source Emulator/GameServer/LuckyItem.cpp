...
void CLuckyItem::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			this->m_LuckyItemInfo.insert(std::pair<int,LUCKY_ITEM_INFO>(info.ItemIndex,info));
...
bool CLuckyItem::GetLuckyItemIndex(LPOBJ lpObj,int section,int group,WORD* ItemIndex) // OK
{
	#if(GAMESERVER_UPDATE>=602)

	for(std::map<int,LUCKY_ITEM_INFO>::iterator it=this->m_LuckyItemInfo.begin();it != this->m_LuckyItemInfo.end();it++)
	{
		if((it->second.ItemIndex/MAX_ITEM_TYPE) == section && it->second.Group == group && gItemManager.CheckItemRequireClass(lpObj,it->second.ItemIndex) != 0)
		{
...
bool CLuckyItem::CharacterUseJewelOfExtension(LPOBJ lpObj,int SourceSlot,int TargetSlot) // OK
{
	...
	if(lpItem->m_Durability < gServerInfo.m_LuckyMinDurabilityRepair)
	{
		return 0;
	}
