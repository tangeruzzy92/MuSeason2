...
void CObjectManager::ObjectSetStateProc() // OK
{
	for(int n=0;n < MAX_OBJECT;n++)
	{
		...
		if(lpObj->Type == OBJECT_MONSTER)
		{
			...
			if(lpObj->Attribute == ATTRIBUTE_CASTLE || lpObj->Attribute == ATTRIBUTE_ACHERON)
			{
...
void CObjectManager::ObjectStateProc(LPOBJ lpObj,int MessageCode,int aIndex,int SubCode) // OK
{
	...
	switch(MessageCode)
	{
		...
		case 4:
			...
			{
				gGuild.gObjGuildWarEnd(lpObj->Guild,lpObj->Guild->TargetGuildNode);
			}
			break;
		case 5:
	...
	for(int n=0;n < MAX_OBJECT;n++)
	{
		if(gObjIsConnected(n) != 0)
		{
			...
			else
			{
				...
				gHelperOffline.MainProc(&gObj[n]);
			}
		}
	}
...
bool CObjectManager::CharacterGameClose(int aIndex) // OK
{
	...
	gGuild.GDGuildMemberDisconnectSend(aIndex);
...
void CObjectManager::CharacterUpdateMapEffect(LPOBJ lpObj) // OK
{
	...
	if(gEffectManager.CheckEffect(lpObj,EFFECT_MANA_SHIELD) != 0)
	{
		CSkill* lpSkill = gSkillManager.GetSkill(lpObj,SKILL_MANA_SHIELD);

		if(lpSkill != 0)
		{
			gSkillManager.UseAttackSkill(lpObj->Index,lpObj->Index,lpSkill);
		}
	}
...
void CObjectManager::CharacterCalcExperienceAlone(LPOBJ lpObj,LPOBJ lpMonster,int damage,int flag,bool type,int AttackDamage) // OK
{
	...
	QWORD aditional = (experience*gServerInfo.m_ExperienceRandomAditional)/100;

	if(aditional>0)
	{
		aditional = GetLargeRand()%aditional;

		if(GetLargeRand()%100 >= 50)
		{
			experience += aditional;
		}
		else
		{
			experience -= aditional;
...
{
	if(strcmp(gServerInfo.m_ServerDebuggerName,lpObj->Name) == 0 && (gServerInfo.m_Debugger == 5 || gServerInfo.m_Debugger == -1))
	{
		Console(LOG_GREEN,"Potion time: (%d)",(GetTickCount()-lpObj->PotionTime));
	}
	...
	if(gServerInfo.m_CheckAutoPotionHack != 0 && (GetTickCount()-lpObj->PotionTime) < ((DWORD)gServerInfo.m_CheckAutoPotionHackTolerance))
	{
		...
		Console(LOG_RED,"[%s][%s] (Potion) Value [%d] Max [%d]",lpObj->Account,lpObj->Name,(int)(GetTickCount()-lpObj->PotionTime),gServerInfo.m_CheckAutoPotionHackTolerance);
...
void CObjectManager::CharacterLifeCheck(LPOBJ lpObj,LPOBJ lpTarget,int damage,int DamageType,int flag,int type,int skill,int ShieldDamage,bool combo) // OK
{
	...
	if(lpTarget->Life <= 0 && lpTarget->Live != 0)
	{
		...
		if(lpTarget->Type == OBJECT_MONSTER)
		{
			...
			if(CF_MAP_RANGE(lpTarget->Map) != 0)
			{
				gChaosCastleFinal.MonsterDieProc(lpTarget,&gObj[SummonIndex]);
			}
