...
void CCastleSiegeCrown::CastleSiegeCrownAct(int aIndex) // OK
{
	...
	if(lpTarget->CsJoinSide != gObj[gCastleSiege.GetCrownSwitchUserIndex(217)].CsJoinSide || lpTarget->CsJoinSide != gObj[gCastleSiege.GetCrownSwitchUserIndex(218)].CsJoinSide)
	{
		...
		gCastleSiege.ResetCrownUserIndex();
		gCastleSiege.ResetCrownAccessTickCount();
		LogAdd(LOG_GREEN,"[CastleSiege] [%s][%s] Register Castle Crown Canceled (GUILD:%s)",lpTarget->Account,lpTarget->Name,lpTarget->GuildName);
		return;
	}

	LogAdd(LOG_GREEN,"[CastleSiege] [Reg. Accumulating] Accumulated Crown AccessTime : acc(%d)+%d [%s](%s)(%s)",lpTarget->AccumulatedCrownAccessTime,(GetTickCount()-gCastleSiege.GetCrownAccessTickCount()),lpTarget->GuildName,lpTarget->Account,lpTarget->Name);
}
