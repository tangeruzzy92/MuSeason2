	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				info.Index = (info.Type == QUEST_WORLD_REWARD_ITEM) ? SafeGetItem(GET_ITEM(lpMemScript->GetAsNumber(), lpMemScript->GetAsNumber())) : lpMemScript->GetAsNumber();
				...
				if(info.Type == QUEST_WORLD_REWARD_ITEM && IsValidItem(info.Index) == 0){Console(LOG_RED,"[%02d-%03d] Invalid ItemIndex. [%s]",info.Index/MAX_ITEM_TYPE,info.Index%MAX_ITEM_TYPE,path);}
			}
		}
	}
	catch (...)
