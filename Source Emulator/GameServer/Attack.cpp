...
bool CAttack::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,int damage,int count,bool combo) // OK
{
	...
	if(lpObj->Map != lpTarget->Map || (gServerInfo.m_TeleportAttackCheck == 0 && lpObj->Teleport != 0))
	{
		return 0;
	}
	...
	{
		if(lpObj->Guild->WarType == GUILD_WAR_TYPE_SOCCER && GetBattleSoccerGoalMove(0) == 0)
		{
			return 1;
		}
...
bool CAttack::CheckPlayerTarget(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(gServerInfo.m_PartyDisableKillBetweenMembers != 0 && gParty.IsMember(lpObj->PartyNumber,lpTarget->Index) != 0)
	{
		return 0;
	}
	...
	if(gGuild.gObjGetRelationShip(lpObj,lpTarget) == 2 && gMapManager.GetMapNonPK(lpTarget->Map) == 0)
	{
		return 1;
	}
	...
	if(gGuild.gObjTargetGuildWarCheck(lpObj,lpTarget) == 0)
	{
		if(lpTarget->Guild != 0 && lpTarget->Guild->WarState != 0)
		{
			...
			if(gCustomArena.CheckMap(lpTarget->Map) == 0 && CC_MAP_RANGE(lpTarget->Map) == 0 && IT_MAP_RANGE(lpTarget->Map) == 0 && gDuel.CheckDuel(lpObj,lpTarget) == 0 && gGensSystem.CheckGens(lpObj,lpTarget) == 0)
			{
				return 0;
			}
		}
...
bool CAttack::MissCheckPvP(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,int send,int count,BYTE* miss) // OK
{
	...
	int AttackSuccessRate = (int)(((100*(((lpObj->AttackSuccessRatePvP*10000.0f)/(lpObj->AttackSuccessRatePvP+lpTarget->DefenseSuccessRatePvP))/10000.0f))*((GAMESERVER_UPDATE<201)?2:gServerInfo.m_ShieldGaugeAttackRate))*(((lpObj->Level*10000.0f)/(lpObj->Level+lpTarget->Level))/10000.0f));
...
int CAttack::GetAttackDamage(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int TargetDefense) // OK
{
	...
	return gSkillDamage.GetDamage(lpSkill,damage);
}
