...
void CGuildMatching::CGGuildMatchingInsertRecv(PMSG_GUILD_MATCHING_INSERT_RECV* lpMsg,int aIndex) // OK
{
	...
	if(gGuildClass.GetGuildMemberStatus(lpGuildInfo->Name,lpObj->Name) != GUILD_STATUS_MASTER)
	{
		pMsg.result = 0xFFFFFFFC;
		DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
		return;
	...
	if(gServerInfo.m_PKLimitFree == 0 && lpObj->PKLevel > 3)
	{
...
void CGuildMatching::DGGuildMatchingJoinAcceptRecv(SDHP_GUILD_MATCHING_JOIN_ACCEPT_RECV* lpMsg) // OK
{
	...
	if(lpMsg->result == 1 && lpMsg->type == 1){gGuild.GDGuildMemberJoinSend(-1,lpMsg->GuildName,lpMsg->MemberName);}
...
void CGuildMatching::DGGuildMatchingNotifyRecv(SDHP_GUILD_MATCHING_NOTIFY_RECV* lpMsg) // OK
{
	...
	if(lpMsg->result == 1){gGuild.GDGuildMemberJoinSend(lpMsg->index,lpMsg->GuildName,lpMsg->name);}
...
void CGuildMatching::GDGuildMatchingInsertSend(int aIndex,char* text,BYTE InterestType,BYTE LevelRange,BYTE ClassType) // OK
{
	...
	pMsg.GuildMasterLevel = ((gObj[aIndex].Level>=gServerInfo.m_MaxLevel)?(gObj[aIndex].Level+gObj[aIndex].MasterLevel):gObj[aIndex].Level);
...
void CGuildMatching::GDGuildMatchingJoinInsertSend(int aIndex,char* GuildName) // OK
{
	...
	pMsg.Level = ((gObj[aIndex].Level>=gServerInfo.m_MaxLevel)?(gObj[aIndex].Level+gObj[aIndex].MasterLevel):gObj[aIndex].Level);
