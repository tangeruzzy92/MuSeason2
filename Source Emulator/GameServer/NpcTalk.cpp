...
void CNpcTalk::NpcCastleSiegeCrown(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	if(gObjIsConnected(iUserIndex) == FALSE )
	{
		...
		if(gObjIsConnected(iSwitchIndex1) == FALSE || gObjIsConnected(iSwitchIndex2) == FALSE )
		{
			...
			LogAdd(LOG_GREEN,"[CastleSiege] [%s][%s] Failed to Register Castle Crown (GUILD:%s)",lpObj->Account,lpObj->Name,lpObj->GuildName);
			return;
		}
		...
		{
			GCAnsCsAccessCrownState(lpObj->Index,4);
			LogAdd(LOG_GREEN,"[CastleSiege] [%s][%s] Failed to Register Castle Crown (GUILD:%s) (S1:%s/%s)(S2:%s/%s)",lpObj->Account,lpObj->Name,lpObj->GuildName,gObj[iSwitchIndex1].Name,gObj[iSwitchIndex1].GuildName,gObj[iSwitchIndex2].Name,gObj[iSwitchIndex2].GuildName);
			return;
		}
		else
		{
			...
			LogAdd(LOG_GREEN,"[CastleSiege] [%s][%s] Start to Register Castle Crown (GUILD:%s)",lpObj->Account,lpObj->Name,lpObj->GuildName);
		}
	}
...
void CNpcTalk::NpcCastleSiegeCrownSwitch(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	if(!gObjIsConnected(iUserIndex))
	{
		...
		LogAdd(LOG_GREEN,"[CastleSiege] [%s][%s] Start to Push Castle Crown Switch (GUILD:%s) - CS X:%d/Y:%d",lpObj->Account,lpObj->Name,lpObj->GuildName,lpNpc->X,lpNpc->Y);
	}
	else if(lpObj->Index != iUserIndex)
	{
...
void CNpcTalk::DGNpcSantaClausRecv(SDHP_NPC_SANTA_CLAUS_RECV* lpMsg) // OK
{
	...
	gItemBagManager.DropItemBySpecialValue(ITEM_BAG_SANTA_CLAUS,-1,-1,lpObj,lpObj->Map,lpObj->X,lpObj->Y);
