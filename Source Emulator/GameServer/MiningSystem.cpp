		...
		while (true)
		{
			...
			while (true)
			{
				...
				else if(section == 1)
				{
					...
					MINING_BONUS_INFO info;

					info.BonusIndex = lpMemScript->GetNumber();

					info.BonusItemRate = lpMemScript->GetAsNumber();

					info.BonusItemIndex = SafeGetItem(GET_ITEM(lpMemScript->GetAsNumber(),lpMemScript->GetAsNumber()));

					info.BonusItemCount = lpMemScript->GetAsNumber();

					this->m_BonusInfo.push_back(info);
				}
				else
				{
					break;
...
void CMiningSystem::CGMiningStartRecv(PMSG_MINING_START_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpObj->Inventory[INVENTORY_SLOT_WEAPON1].IsItem() == 0 || lpObj->Inventory[INVENTORY_SLOT_WEAPON1].m_Index != GET_ITEM(0,41) || lpObj->Inventory[INVENTORY_SLOT_WEAPON1].m_Durability < MiningInfo.Value)
	{
	...
	{
		lpObj->Inventory[INVENTORY_SLOT_WEAPON1].m_Durability = (lpObj->Inventory[INVENTORY_SLOT_WEAPON1].m_Durability-MiningInfo.Value);

		gItemManager.GCItemDurSend(aIndex,0,(BYTE)lpObj->Inventory[INVENTORY_SLOT_WEAPON1].m_Durability,0);
...
void CMiningSystem::CGMiningSuccessRecv(PMSG_MINING_SUCCESS_RECV* lpMsg,int aIndex) // OK
{
	...
	for(int n=0;n < MAX_JEWEL_MIX_BUNDLE;n++)
	{
		...
		{
			if(lpObj->MiningStage == MAX_MINING_STAGE){ GCServerCommandSend(lpObj->Index,0,lpObj->X,lpObj->Y,1);}
			this->CreateMiningReward(aIndex,MiningInfo.ItemIndex,gJewelMix.GetJewelBundleIndex(n),MiningInfo.ItemCount);
			break;
		}
	}

	if(MiningInfo.BonusIndex != -1)
	{
		for(std::vector<MINING_BONUS_INFO>::iterator it = this->m_BonusInfo.begin(); it != this->m_BonusInfo.end(); it++)
		{
			if(it->BonusIndex != MiningInfo.BonusIndex)
			{
				continue;
			}

			if(it->BonusItemRate >= 0 && (GetLargeRand() % 10000) < it->BonusItemRate)
			{
				for(int n = 0; n < MAX_JEWEL_MIX_BUNDLE; n++)
				{
					if(gJewelMix.GetJewelSimpleIndex(n) == it->BonusItemIndex)
					{
						gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(277),lpObj->Name,it->BonusItemCount,gItemManager.GetItemName(it->BonusItemIndex,0));
						this->CreateMiningReward(aIndex,it->BonusItemIndex,gJewelMix.GetJewelBundleIndex(n),it->BonusItemCount);
						break;
					}
				}
			}
	...
	gObjectManager.CharacterLifeCheck(lpObj,&gObj[lpMsg->MonsterIndex],0,0,0,0,0,0,0);
