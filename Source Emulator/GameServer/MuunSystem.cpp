		...
		while(true)
		{
			...
			while(true)
			{
				if(section == 0)
				{
					...
					int EvoIndex = lpMemScript->GetAsNumber();

					info.EvolutionItemIndex = (EvoIndex != -1) ? SafeGetItem(GET_ITEM(EvoIndex,lpMemScript->GetAsNumber())) : EvoIndex;

					this->m_MuunSystemInfo.insert(std::pair<int,MUUN_SYSTEM_INFO>(info.ItemIndex,info));
