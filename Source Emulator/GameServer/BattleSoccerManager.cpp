...
{
	if( BATTLE_SOCCER_GROUND_RANGE(ground)  == 0 )
		return 0;
	...
	if(gGuild.gObjGuildWarProc(lpGuild,lpTargetGuild,score) == 1 )
	{
		if( lpGuild->WarType == GUILD_WAR_TYPE_SOCCER)
		{
			gGuild.gObjGuildWarEndSend(lpGuild,lpTargetGuild,1,0);
			gBattleGroundEnable(lpGuild->BattleGroundIndex,0);
			gGuild.gObjGuildWarEnd(lpGuild,lpTargetGuild);
		}
	}
