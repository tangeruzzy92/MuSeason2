...
void CSkillManager::UseAttackSkill(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	if(lpObj->Type == OBJECT_USER && lpSkill->m_skill == SKILL_TWISTING_SLASH && lpObj->Inventory[INVENTORY_SLOT_WEAPON1].IsItem() == 0)
	{
		return;
	}
	...
	if(lpObj->Type == OBJECT_USER && (lpObj->Class == CLASS_MG || lpObj->Class == CLASS_DL || lpObj->Class == CLASS_RF || gQuest.CheckQuestListState(lpObj,((lpObj->Class==CLASS_DK)?3:2),QUEST_FINISH) != 0))
	{
		if(lpObj->ComboSkill.CheckCombo(lpObj->Class,lpSkill->m_skill) != 0)
		{
			...
			else if(gServerInfo.m_CheckAutoComboHack != 0 && (GetTickCount()-lpObj->ComboTime) < ((DWORD)gServerInfo.m_CheckAutoComboHackTolerance))
			{
				...
				Console(LOG_RED,"[%s][%s] (Combo) Value [%d] Max [%d]",lpObj->Account,lpObj->Name,(int)(GetTickCount()-lpObj->ComboTime),gServerInfo.m_CheckAutoComboHackTolerance);
...
bool CSkillManager::SkillManaShield(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	if(CF_MAP_RANGE(lpObj->Map) != 0 && aIndex != bIndex)
	{
		return 0;
	}
...
bool CSkillManager::SkillSummon(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	if(lpObj->Map == MAP_ICARUS || CC_MAP_RANGE(lpObj->Map) != 0 || CF_MAP_RANGE(lpObj->Map) != 0)
	{
		return 0;
	}
