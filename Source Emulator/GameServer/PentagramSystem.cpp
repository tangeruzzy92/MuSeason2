		...
		while(true)
		{
			...
			while(true)
			{
				if(section == 0)
				{
					...
					this->m_PentagramTypeInfo.insert(std::pair<int,PENTAGRAM_TYPE_INFO>(info.ItemIndex,info));
...
bool CPentagramSystem::CheckPentagramOption(LPOBJ lpObj,PENTAGRAM_OPTION_INFO* lpInfo) // OK
{
	...
	for(int n=0;n < MAX_SOCKET_OPTION;n++)
	{
		if(lpInfo->Type[n] != -1)
		{
			PENTAGRAM_JEWEL_INFO* lpPentagramJewelInfo = this->GetPentagramJewelInfo(lpObj,lpObj->Inventory[INVENTORY_SLOT_PENTAGRAM].m_SocketOption[n],PENTAGRAM_JEWEL_TYPE_INVENTORY);
...
void CPentagramSystem::CalcPentagramOption(LPOBJ lpObj,bool flag) // OK
{
	...
	if(lpObj->Inventory[INVENTORY_SLOT_PENTAGRAM].IsItem() == 0 || lpObj->Inventory[INVENTORY_SLOT_PENTAGRAM].IsPentagramItem() == 0 || lpObj->Inventory[INVENTORY_SLOT_PENTAGRAM].m_IsValidItem == 0)
	{
		return;
	}
	...
	if(this->GetPentagramTypeInfo(lpObj->Inventory[INVENTORY_SLOT_PENTAGRAM].m_Index,&PentagramTypeInfo) != 0)
	{
