...
void CEffectManager::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			if(IsValidItem(info.ItemIndex) == 0 && info.ItemIndex != -1){Console(LOG_RED,"[%02d-%03d] Invalid ItemIndex. [%s]",info.ItemIndex/MAX_ITEM_TYPE,info.ItemIndex%MAX_ITEM_TYPE,path);}
...
int CEffectManager::GenerateEffectList(LPOBJ lpObj,BYTE* lpMsg,int* size) // OK
{
	...
	for(int n=0;n < MAX_EFFECT_LIST;n++)
	{
		...
		{
			if(lpObj->Effect[n].m_index == EFFECT_GAME_MASTER && (gCustomArena.CheckMap(lpObj->Map) != 0 || KALIMA_MAP_RANGE(lpObj->Map) != 0 || DS_MAP_RANGE(lpObj->Map) != 0 || BC_MAP_RANGE(lpObj->Map) != 0 || CC_MAP_RANGE(lpObj->Map) != 0 || IT_MAP_RANGE(lpObj->Map) != 0 || DA_MAP_RANGE(lpObj->Map) != 0 || DG_MAP_RANGE(lpObj->Map) != 0 || IG_MAP_RANGE(lpObj->Map) != 0 || CF_MAP_RANGE(lpObj->Map) != 0))
			{
				continue;
			}
	...
	for(int n=0;n < MAX_EFFECT_LIST;n++)
	{
		if(lpObj->Effect[n].IsEffect() != 0)
		{
			...
			if(this->GetEffectState(lpObj->Effect[n].m_index,&result) != 0)
			{
				count |= result;
			}
		}
...
void CEffectManager::GCEffectStateSend(LPOBJ lpObj,BYTE state,BYTE effect) // OK
{
	...
	if(this->GetEffectState(effect,&pMsg.effect) == 0)
	{
		return;
	}
...
{
	#if((GAMESERVER_UPDATE==201 && GAMESERVER_LANGUAGE==5) || GAMESERVER_UPDATE==301)

	this->GetPeriodicEffect(effect,&group,&value);
...
bool CEffectManager::GetEffectState(BYTE effect,DWORD* result) // OK
{
...
void CEffectManager::GeneratePeriodicEffectList(LPOBJ lpObj) // OK
{
	#if((GAMESERVER_UPDATE==201 && GAMESERVER_LANGUAGE==5) || GAMESERVER_UPDATE==301)
	...
	for(int n=0;n < MAX_EFFECT_LIST;n++)
	{
		...
		this->GCPeriodicEffectSend(lpObj,0,0,0,lpEffect->m_count,lpEffect->m_index);
	}

	#endif
}

void CEffectManager::GetPeriodicEffect(BYTE effect,WORD* option,WORD* type) // OK
{
	#if((GAMESERVER_UPDATE==201 && GAMESERVER_LANGUAGE==5) || GAMESERVER_UPDATE==301)
