...
void CParty::CGPartyRequestResultRecv(PMSG_PARTY_REQUEST_RESULT_RECV* lpMsg,int aIndex) // OK
{
	...
	if(gCustomArena.CheckMap(lpObj->Map) != 0 || CC_MAP_RANGE(lpObj->Map) != 0 || IT_MAP_RANGE(lpObj->Map) != 0 || DA_MAP_RANGE(lpObj->Map) != 0 || DG_MAP_RANGE(lpObj->Map) != 0 || IG_MAP_RANGE(lpObj->Map) != 0 || CF_MAP_RANGE(lpObj->Map) != 0)
	{
		this->GCPartyResultSend(aIndex,0);
		goto CLEAR_JUMP;
	}
