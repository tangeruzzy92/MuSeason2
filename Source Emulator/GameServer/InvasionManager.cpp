		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 1)
				{
					...
					if(IsValidMonster(this->m_InvasionInfo[index].BossIndex) == 0){Console(LOG_RED,"[%d] Invalid BossIndex. [%s]",this->m_InvasionInfo[index].BossIndex,path);}
				}
				else if(section == 2)
				{
					if(strcmp("end",lpMemScript->GetAsString()) == 0)
		...
		if(lpInfo->RespawnInfo[n].empty() == 0)
		{
			...
			for(std::vector<INVASION_MONSTER_INFO>::iterator it=lpInfo->MonsterInfo.begin();it != lpInfo->MonsterInfo.end();it++)
			{
				...
				for(int n=0;n < it->Count;n++)
				{
					...
					LogAdd(LOG_RED,"[Invasion Manager] (%d) SpawnMonster [%d] [%s][%d][%d]",lpInfo->Index,it->MonsterClass,gMapManager.GetMapName(lpObj->Map),lpObj->X,lpObj->Y);
				}
			}
		}
	}
...
void CInvasionManager::CheckSync(INVASION_INFO* lpInfo) // OK
{
	...
	LogAdd(LOG_RED,"[Invasion Manager] (%d) Sync Start Time. [%d] min remain",lpInfo->Index,(lpInfo->RemainTime/60));
}
