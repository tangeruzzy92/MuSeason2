...
bool CSummonScroll::CreateSummonScrollMonster(int aIndex,int ItemIndex,int map,int x,int y) // OK
{
	...
	if(DS_MAP_RANGE(lpObj->Map) != 0 || BC_MAP_RANGE(lpObj->Map) != 0 || CC_MAP_RANGE(lpObj->Map) != 0 || IT_MAP_RANGE(lpObj->Map) != 0 || DA_MAP_RANGE(lpObj->Map) != 0 || DG_MAP_RANGE(lpObj->Map) != 0 || IG_MAP_RANGE(lpObj->Map) != 0 || CF_MAP_RANGE(lpObj->Map) != 0)
	{
		return 0;
	}
	...
	for(int n=0;n < MAX_SUMMON_SCROLL_MONSTER_GROUP;n++)
	{
		if(RandomManager[n].GetRandomElement((int*)&lpSummonScrollInfo) != 0)
		{
			...
			if(gObjGetRandomMonsterLocation(map,&px,&py,3,3,50) == 0)
			{
				px = x;
				py = y;
			}
