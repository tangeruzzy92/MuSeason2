...
void CChaosCastle::ProcState_STAND(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	...
	{
		LogAdd(LOG_BLUE,"[Chaos Castle] (%d) Not enough users",(lpLevel->Level+1));
...
void CChaosCastle::SetState_EMPTY(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	...
	LogAdd(LOG_BLUE,"[Chaos Castle] (%d) SetState EMPTY",(lpLevel->Level+1));
}
...
void CChaosCastle::SetState_STAND(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	...
	LogAdd(LOG_BLUE,"[Chaos Castle] (%d) SetState STAND",(lpLevel->Level+1));
}
...
void CChaosCastle::SetState_START(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	...
	LogAdd(LOG_BLUE,"[Chaos Castle] (%d) SetState START",(lpLevel->Level+1));
}
...
void CChaosCastle::SetState_CLEAN(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	...
	LogAdd(LOG_BLUE,"[Chaos Castle] (%d) SetState CLEAN",(lpLevel->Level+1));
}
...
void CChaosCastle::CheckSync(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	...
	LogAdd(LOG_BLUE,"[Chaos Castle] (%d) Sync Start Time. [%d] min remain",(lpLevel->Level+1),(lpLevel->RemainTime/60));
}
...
void CChaosCastle::GiveUserRewardItem(CHAOS_CASTLE_LEVEL* lpLevel,int aIndex) // OK
{
	...
	gItemBagManager.DropItemBySpecialValue(ITEM_BAG_CHAOS_CASTLE,lpLevel->Level,1,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);
}
