...
void CViewport::DestroyViewportGuild(int aIndex,int type) // OK
{
	...
	pMsg.index[0] = SET_NUMBERHB(aIndex) & 0x7F;

	pMsg.index[1] = SET_NUMBERLB(aIndex);

	if(type != 0)
	{
		pMsg.index[0] |= GUILD_STATUS_MASTER;
	}
...
void CViewport::GCViewportPlayerSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if(((lpTarget->Authority & 32) != 0 || gGameMaster.GetGameMasterLevel(lpTarget) != -1) && gEffectManager.CheckEffect(lpTarget,EFFECT_INVISIBILITY) != 0)
		{
			continue;
		}
		...
		lpTarget->CharSet[0] |= lpTarget->ViewState & 15;
...
void CViewport::GCViewportGuildSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if(lpObj->Guild != 0)
		{
			info.relationship = gGuild.gObjGetRelationShip(lpObj,lpTarget);
		}
		else
		{
			info.relationship = 0;
...
void CViewport::GCViewportUnionSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		gGuild.gObjGetGuildUnionName(lpTarget,info.UnionName,sizeof(info.UnionName));
...
void CViewport::GCViewportSimplePlayerSend(LPOBJ lpObj) // OK
{
	...
	if(((lpObj->Authority & 32) != 0 || gGameMaster.GetGameMasterLevel(lpObj) != -1) && gEffectManager.CheckEffect(lpObj,EFFECT_INVISIBILITY) != 0)
	{
		return;
	}
	...
	lpObj->CharSet[0] |= lpObj->ViewState & 15;
...
void CViewport::GCViewportSimpleUnionSend(LPOBJ lpObj) // OK
{
	...
	gGuild.gObjGetGuildUnionName(lpObj,info.UnionName,sizeof(info.UnionName));
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		if(lpObj->VpPlayer2[n].state != VIEWPORT_NONE && lpObj->VpPlayer2[n].type == OBJECT_USER)
		{
			...
			{
				info.relationship = gGuild.gObjGetRelationShip(&gObj[lpObj->VpPlayer2[n].index],lpObj);
