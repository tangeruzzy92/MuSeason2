...
void CCustomArena::SetState_CLEAN(CUSTOM_ARENA_INFO* lpInfo) // OK
{
	...
	for(int n=0;n < MAX_CUSTOM_ARENA_USER;n++)
	{
		if(OBJECT_RANGE(lpInfo->User[n].Index) != 0)
		{
			...
			gItemBagManager.DropItemBySpecialValue(ITEM_BAG_CUSTOM_ARENA,0,lpInfo->User[n].Rank,&gObj[lpInfo->User[n].Index],gObj[lpInfo->User[n].Index].Map,gObj[lpInfo->User[n].Index].X,gObj[lpInfo->User[n].Index].Y);
