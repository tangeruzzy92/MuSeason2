...
bool CHackPacketCheck::CheckPacketHack(int aIndex,int index,int value,int encrypt,int serial) // OK
{
	...
	if(lpInfo == 0)
	{
		...
		Console(LOG_GREEN,"[%s][%s] Packet unknown error (Index: %d,Value: %d)",gObj[aIndex].Account,gObj[aIndex].Name,index,value);
	...
	if(lpInfo->Encrypt >= 0 && lpInfo->Encrypt != encrypt)
	{
		...
		Console(LOG_GREEN,"[%s][%s] Packet encryption error (Index: %d,Value: %d,Encrypt: [%d][%d])",gObj[aIndex].Account,gObj[aIndex].Name,index,lpInfo->Value,encrypt,lpInfo->Encrypt);
		CloseClient(aIndex);return 0;
	}
	...
	if(serial >= 0 && gSerialCheck[aIndex].CheckSerial(serial) == 0)
	{
		...
		Console(LOG_GREEN,"[%s][%s] Packet serial error (Index: %d,Value: %d,Serial: [%d][%d])",gObj[aIndex].Account,gObj[aIndex].Name,index,lpInfo->Value,serial,gSerialCheck[aIndex].GetRecvSerial());
		CloseClient(aIndex);return 0;
	}
	...
	if(this->CheckPacketHackMinCount(aIndex,index,lpInfo->MinCount) == 0)
	{
		...
		Console(LOG_GREEN,"[%s][%s] Packet count error (Index: %d,Value: %d,Count: [%d][%d])",gObj[aIndex].Account,gObj[aIndex].Name,index,lpInfo->Value,gObj[aIndex].HackPacketCount[index],lpInfo->MaxCount);
