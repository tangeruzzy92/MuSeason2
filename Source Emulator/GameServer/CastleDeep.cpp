		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 3)
				{
					...
					if(IsValidMonster(info.Class) == 0){Console(LOG_RED,"[%d] Invalid MonsterIndex. [%s]",info.Class,path);}
				}
				else
				{
					break;
...
void CCastleDeep::SetState_EMPTY() // OK
{
	...
	LogAdd(LOG_GREEN,"[Castle Deep] SetState EMPTY");
}
...
void CCastleDeep::SetState_START() // OK
{
	...
	LogAdd(LOG_GREEN,"[Castle Deep] SetState START");
}
...
void CCastleDeep::CheckSync() // OK
{
	...
	LogAdd(LOG_GREEN,"[Castle Deep] Sync Start Time. [%d] min remain",(this->m_RemainTime/60));
}
