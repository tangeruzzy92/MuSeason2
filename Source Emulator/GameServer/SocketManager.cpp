...
bool CSocketManager::DataRecv(int index,IO_MAIN_BUFFER* lpIoBuffer) // OK
{
	...
	while(true)
	{
		...
		if(size <= lpIoBuffer->size)
		{
			...
			if(gServerInfo.m_MaxPacketPerSecond != 0 && (++lpObj->PacketCount > gServerInfo.m_MaxPacketPerSecond))
			{
				gLog.Output(LOG_HACK,"[%s] Client disconnected, packets exceeded. [%d][%d]",lpObj->IpAddr,lpObj->PacketCount,gServerInfo.m_MaxPacketPerSecond);
...
void CSocketManager::OnRecv(int index,DWORD IoSize,IO_RECV_CONTEXT* lpIoContext) // OK
{
	...
	{
		gLog.Output(LOG_HACK,"[DataRecv] MainBuffer Error (Index: %d,IpAddress: %s HardwareId: %s)",index,gObj[index].IpAddr,gObj[index].HardwareId);
		this->Disconnect(index);
		this->m_critical.unlock();
		return;
	}
