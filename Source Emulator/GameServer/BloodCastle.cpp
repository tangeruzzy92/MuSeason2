...
void CBloodCastle::ProcState_STAND(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	{
		LogAdd(LOG_RED,"[Blood Castle] (%d) Not enough users",(lpLevel->Level+1));
...
void CBloodCastle::SetState_EMPTY(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	LogAdd(LOG_RED,"[Blood Castle] (%d) SetState EMPTY",(lpLevel->Level+1));
}
...
void CBloodCastle::SetState_STAND(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	LogAdd(LOG_RED,"[Blood Castle] (%d) SetState STAND",(lpLevel->Level+1));
}
...
void CBloodCastle::SetState_START(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	LogAdd(LOG_RED,"[Blood Castle] (%d) SetState START",(lpLevel->Level+1));
}
...
void CBloodCastle::SetState_CLEAN(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	LogAdd(LOG_RED,"[Blood Castle] (%d) SetState CLEAN",(lpLevel->Level+1));
}
...
void CBloodCastle::CheckSync(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	LogAdd(LOG_RED,"[Blood Castle] (%d) Sync Start Time. [%d] min remain",(lpLevel->Level+1),(lpLevel->RemainTime/60));
}
...
void CBloodCastle::GiveUserRewardItem(BLOOD_CASTLE_LEVEL* lpLevel,int aIndex) // OK
{
	...
	gItemBagManager.DropItemBySpecialValue(ITEM_BAG_BLOOD_CASTLE,lpLevel->Level,1,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);
}
