...
void CMonsterManager::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			info.Resistance[4] = lpMemScript->GetAsNumber();

			info.Resistance[5] = lpMemScript->GetAsNumber();

			info.Resistance[6] = lpMemScript->GetAsNumber();
